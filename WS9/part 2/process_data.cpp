...
namespace sdds
{
	...
	ProcessData::ProcessData(const std::string& filename, int n_threads) {  
		...
		inFile.read(reinterpret_cast<char*>(&total_items), sizeof(total_items));
		
		data = new int[total_items];
		
		inFile.read(reinterpret_cast<char*>(data), sizeof(total_items) * total_items);
	...
	int ProcessData::operator()(const std::string& target_file, double& avg, double& var) {
		...
		std::fstream outFile{};
		outFile.open(target_file, std::ios_base::out | std::ios_base::binary);

		if (!outFile.is_open()) {
			std::cerr << "Error: Unable to open file " << target_file << " for writing" << std::endl;
			throw std::runtime_error("Unable to open " + target_file + " for writing");
		}

		outFile.write(reinterpret_cast<char *>(&total_items), sizeof(int));
		outFile.write(reinterpret_cast<char *>(data), total_items * sizeof(data[0]));
