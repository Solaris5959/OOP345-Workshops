	...
	struct Profile {
		...
		void validateAddress()
		{
			if (m_address.number < 0)
				throw std::string("Invalid address number: " + std::to_string(m_address.number));

			std::regex postalCodeRegex("^\\b[A-Za-z]\\d[A-Za-z] ?\\d[A-Za-z]\\d\\b$");
			if (!std::regex_match(m_address.postal_code, postalCodeRegex))
			{
